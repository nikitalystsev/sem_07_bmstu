...
import checks
from kolmogorov_eq_solver import KolmogorovEqSolver
...
class MyWindow(tk.Tk):
    ...
    def __init__(self):
        ...
        self._var_input_cnt_states.trace_add(
            "write", self.__display_intensity_matrix)

        self._lbl_input_cnt_states = self.__create_label(
            master=self._frame_widgets, text="Количество состояний")
        ...
        self._entry_input_cnt_states.config(
            font=(self.cfgWin.FONT, 25, self.cfgWin.FONT_STYLE), width=7)
        self._entry_input_cnt_states.config(
            textvariable=self._var_input_cnt_states)
        self._entry_input_cnt_states.grid(
