class BufferMemory:
    """
    Класс, реализующий компонент буферной памяти СМО (она же очередь)
    """
    ...
    def write_request(self, x: float):
        """
        ...
        self._cur_len += 1
        self._max_len = max(self._max_len, self._cur_len)
    ...
    def get_max_queue_len(self):
        """
        Метод получает максимаьльную очередь
        """
...
#         self._pust = False  # признак отстутсвия сообщений
