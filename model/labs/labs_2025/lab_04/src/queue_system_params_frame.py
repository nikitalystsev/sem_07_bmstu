...
class QueueSystemParamsFrame(tk.Frame):
    ...
    def create_widgets(self):
        ...
        for i, (name) in enumerate(self.parameters):
            label_name = tk.Label(self, text=name, font=(
                ConfigGUI.FONT, 18, ConfigGUI.FONT_STYLE), justify='left', anchor="w", background="#3D517F", foreground=ConfigGUI.WHITE)
            label_name.grid(row=i, column=0, columnspan=4,
            ...
            entry.grid(row=i + 1, column=0, columnspan=4,
                       sticky="wens", padx=10, pady=5)
